...
const int BEAM_STEPS = 80;
const int CHILDREN_PER_STATE = 40;
const int MAX_CANDIDATES_TO_VALIDATE = 800;
...
vector<PlacedBay> generate_add_children(
    const vector<PlacedBay>& sol,
    ...
    vector<PlacedBay> base = sol;
    vector<vector<PlacedBay>> children;
    vector<PlacedBay> result_holder;

    int validate_limit = min(MAX_CANDIDATES_TO_VALIDATE, (int)candidates.size());

    for (int i = 0; i < validate_limit && (int)children.size() < max_children; i++) {
        auto& c = candidates[i];
        ...
        if (valid_candidate(
            ...
            vector<PlacedBay> child = sol;
            child.push_back(make_candidate(c.t, c.x, c.y, c.angle));
            children.push_back(child);
        }
    }

    sort(children.begin(), children.end(), [&](const vector<PlacedBay>& a, const vector<PlacedBay>& b) {
        return quality(a, wh_area) < quality(b, wh_area);
    });
    ...
    // Flatten no, returned as vector of children impossible with current signature,
    // so this function will be replaced below.
    return {};
}

vector<vector<PlacedBay>> generate_children(
    const vector<PlacedBay>& sol,
    const vector<BayType>& types,
    const vector<Point>& warehouse,
    const vector<Obstacle>& obstacles,
...
vector<PlacedBay> best_solution_from_beam(const vector<vector<PlacedBay>>& beam, double wh_area) {
    if (beam.empty()) return {};

    int best_idx = 0;
    double best_q = quality(beam[0], wh_area);

    for (int i = 1; i < (int)beam.size(); i++) {
        double q = quality(beam[i], wh_area);
...
vector<PlacedBay> beam_search(
    ...
    vector<vector<PlacedBay>> beam;
    beam.push_back({});

    vector<vector<PlacedBay>> last_non_empty_beam = beam;

    for (int step = 0; step < BEAM_STEPS; step++) {
        vector<vector<PlacedBay>> all_children;
        ...
        if (all_children.empty()) {
            cout << "Beam stopped at step " << step << "\n";
            break;
        }

        sort(all_children.begin(), all_children.end(), [&](const vector<PlacedBay>& a, const vector<PlacedBay>& b) {
            double qa = quality(a, wh_area);
            double qb = quality(b, wh_area);
        ...
        int limit = min(BEAM_WIDTH, (int)all_children.size());

        for (int i = 0; i < limit; i++) {
            beam.push_back(all_children[i]);
        }

        last_non_empty_beam = beam;

        auto best_level = best_solution_from_beam(beam, wh_area);
        auto [a, l, pr, q] = details(best_level, wh_area);

        cout << "Beam step " << step + 1
             << " | children=" << all_children.size()
             << " | best_level_bays=" << best_level.size()
             << " | area=" << a
             << " | loads=" << l
             << " | price=" << pr
             << " | Q=" << q
    ...
    return best_solution_from_beam(last_non_empty_beam, wh_area);
}

void solve_case(const string& case_dir) {
    cout << "\n=== Beam Solving " << case_dir << " ===\n";
    ...
    auto sol = beam_search(types, warehouse, obstacles, ceiling, wh_area);

    bool valid = is_valid_solution(sol, warehouse, obstacles, ceiling);

    if (!valid) {
        cerr << "ERROR: invalid final solution. Writing empty solution.\n";
        sol.clear();
    }
